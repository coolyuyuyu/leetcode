...
class Solution {
public:
    vector<vector<string>> accountsMerge_UnionFind(vector<vector<string>>& accounts) {
        unordered_map<string, string> mail2Person;
        DisjointSets<string, map<string, string>> ds;
        for (const auto& account : accounts) {
            mail2Person[account[1]] = account[0];
            ds.make_set(account[1]);

            for (size_t i = 2; i < account.size(); ++i) {
                mail2Person[account[i]] = account[0];
                ds.merge(account[i - 1], account[i]);
            }
        }

        accounts.clear();
        for (vector<string> mails : ds.sets()) {
            vector<string> account;
            account.push_back(mail2Person[mails[0]]);
            account.insert(account.end(), make_move_iterator(mails.begin()), make_move_iterator(mails.end()));
    ...
    vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
        return accountsMerge_UnionFind(accounts);
    }
};
