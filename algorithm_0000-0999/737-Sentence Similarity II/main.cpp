...
class Solution {
public:
    bool areSentencesSimilarTwo_UnionFind(vector<string>& words1, vector<string>& words2, vector<vector<string>>& pairs) {
        if (words1.size() != words2.size()) {
            return false;
        }
    
        DisjointSets<string, unordered_map<string, string>> ds;
        for (const auto& p : pairs) {
            ds.merge(p[0], p[1]);
        }
        
        for (size_t i = 0; i < words1.size(); ++i) {
            if (words1[i] != words2[i] && !ds.connected(words1[i], words2[i])) {
                return false;
            }
        }
    ...
    bool areSentencesSimilarTwo(vector<string>& words1, vector<string>& words2, vector<vector<string>>& pairs) {
        return areSentencesSimilarTwo_UnionFind(words1, words2, pairs);    
    }
};
