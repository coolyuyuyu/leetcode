...
public:
    void solve_UnionFind(vector<vector<char>>& board) {
        unsigned short rowCnt = board.size();
        unsigned short colCnt = board.front().size();

        auto getId = [&rowCnt, &colCnt](unsigned short row, unsigned short col) {
            return (row * colCnt + col);
        };
        unsigned short boundaryId = rowCnt * colCnt;

        //DisjointSets<unsigned short> ds;
        DisjointSets<unsigned short, SequenceWrapper<vector<unsigned short>>> ds;
        for (unsigned short row = 0; row < rowCnt; ++row) {
            for (unsigned short col = 0; col < colCnt; ++col) {
                if (board[row][col] == 'X') {
                    continue;
                }

                unsigned short id = getId(row, col);
                ds.make_set(id);
                if (row == 0 || col == 0 || (row + 1) == rowCnt || (col + 1) == colCnt) {
                    ds.merge(id, boundaryId);
                }

                if (0 < col && board[row][col - 1] == 'O') { // lft
                    ds.merge(id, getId(row, col - 1));
                }
                if (0 < row && board[row - 1][col] == 'O') { // top
