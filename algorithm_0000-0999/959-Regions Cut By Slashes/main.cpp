...
class Solution {
public:
    int regionsBySlashes_UnionFind(vector<string>& grid) {
        unsigned short rowCnt = grid.size();
        unsigned short colCnt = grid.front().size();

        auto getId = [&rowCnt, &colCnt](unsigned short row, unsigned short col) {
            return (row * colCnt + col) * 4;
        };
        ...
        for (unsigned short row = 0; row < rowCnt; ++row) {
            for (unsigned short col = 0; col < colCnt; ++col) {
                ...
                if (0 < row) { // top
                    ds.merge(id + 1, getId(row - 1, col) + 3);
                }

                if (0 < col) { // lft
                    ds.merge(id + 0, getId(row, col - 1) + 2);
                }
            }
        }
    ...
    int regionsBySlashes(vector<string>& grid) {
        return regionsBySlashes_UnionFind(grid);
    }
};
