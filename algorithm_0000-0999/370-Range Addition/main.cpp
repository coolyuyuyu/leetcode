...
class Solution {
public:
    vector<int> byDiffArray(int length, vector<vector<int>>& updates) {
    ...
    vector<int> bySegmentTree(int length, vector<vector<int>>& updates) {
        DiffSegmentTree<int> st(length, 0);
        for (const auto& update : updates) {
            st.update(update[0], update[1], update[2]);
        }
    ...
    vector<int> getModifiedArray(int length, vector<vector<int>>& updates) {
        //return byDiffArray(length, updates);
        return bySegmentTree(length, updates);
    }
};
