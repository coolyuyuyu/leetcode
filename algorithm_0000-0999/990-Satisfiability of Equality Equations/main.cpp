...
class Solution {
public:
    bool equationsPossible_UnionFind(vector<string>& equations) {
        vector<pair<char, char>> inequalities;
        DisjointSets<char> ds;

        for (const string& equation : equations) {
            if (equation[1] == '=') {
                if (equation[0] != equation[3]) {
                    ds.merge(equation[0], equation[3]);
                }
            }
            else {
                inequalities.emplace_back(equation[0], equation[3]);
            }
        }

        for (const pair<char, char>& p : inequalities) {
            if (p.first == p.second) {
                return false;
            }
            else {
                if (ds.connected(p.first, p.second)) {
                    return false;
                }
            }
        }
    ...
    bool equationsPossible(vector<string>& equations) {
        return equationsPossible_UnionFind(equations);
    }
};
