...
class Solution {
public:
    int minCostConnectPoints_UnionFind(vector<vector<int>>& points) {
        class IdMapper{
        public:
            int operator()(int x, int y) {
                return (m.emplace(make_pair(x, y), m.size()).first->second);
            }
        private:
        ...
        //DisjointSets<int> ds;
        DisjointSets<unsigned, SequenceWrapper<vector<unsigned>>, FindNoCompress<SequenceWrapper<vector<unsigned>>>> ds;
        priority_queue<Edge, vector<Edge>, greater<Edge>> pq;
        for (size_t i = 0; i < points.size(); ++i) {
            int id1 = mapper(points[i][0], points[i][1]);
            ds.make_set(id1);
            for (size_t j = i + 1; j < points.size(); ++j) {
                int id2 = mapper(points[j][0], points[j][1]);
                int w = abs(points[i][0] - points[j][0]) + abs(points[i][1] - points[j][1]);
                pq.emplace(id1, id2, w);
            }
        }
