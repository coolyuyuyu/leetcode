...
class Solution {
public:
    int minimumCost_UnionFind(int N, vector<vector<int>>& connections) {
        ...
        struct Edge {
            ...
            Edge(unsigned short node1_, unsigned short node2_, int weight_)
                : node1(node1_)
                , node2(node2_)
                , weight(weight_) {
            }
        ...
        for (const auto& connection : connections) {
            pq.emplace(connection[0], connection[1], connection[2]);
        }
        ...
        while (1 < ds.size() && !pq.empty()) {
            ...
            if (ds.merge(e.node1, e.node2)) {
                cost += e.weight;
            }
        }

        return (1 < ds.size() ? -1 : cost);
    }

    int minimumCost(int N, vector<vector<int>>& connections) {
        return minimumCost_UnionFind(N, connections);
    }
};