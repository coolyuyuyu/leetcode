...
class Solution {
public:
    int makeConnected_UnionFind(int n, vector<vector<int>>& connections) {
        ...
        for (const auto& connection : connections) {
            if (!ds.merge(connection[0], connection[1])) {
                ++redundant;
            }
        }

        return (ds.size() <= (redundant + 1) ? ds.size() - 1 : -1);
    }

    int makeConnected(int n, vector<vector<int>>& connections) {
        return makeConnected_UnionFind(n, connections);
    }
};