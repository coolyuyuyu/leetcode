...
class Solution {
public:
    int minimumEffortPath_UnionFind(vector<vector<int>>& heights) {
        int rowCnt = heights.size();
        int colCnt = heights.empty() ? 0 : heights.front().size();
        assert(0 < rowCnt && 0 < colCnt);

        function<int(int, int)> getId = [&rowCnt, &colCnt](int row, int col) {
            return row * colCnt + col;
        };
        int srcId = getId(0, 0), dstId = getId(rowCnt - 1, colCnt - 1);
        ...
        struct Edge {
            ...
            Edge(int id1_, int id2_, int diff_)
                : id1(id1_)
                , id2(id2_)
                , diff(diff_) {
            }
        ...
        for (int row = 0; row < rowCnt; ++row) {
            for (int col = 0; col < colCnt; ++col) {
                if ((row + 1) < rowCnt) {
                    ...
                    pq.emplace(getId(row, col), getId(row + 1, col), diff);
                }

                if ((col + 1) < colCnt) {
                    int diff = abs(heights[row][col] - heights[row][col + 1]);
                    pq.emplace(getId(row, col), getId(row, col + 1), diff);
                }
            }
        }
    ...
    int minimumEffortPath(vector<vector<int>>& heights) {
        return minimumEffortPath_UnionFind(heights);
    }
};