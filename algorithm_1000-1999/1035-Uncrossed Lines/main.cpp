class Solution {
public:
    int topdn_dp(const vector<int>& s, size_t i, const vector<int>& t, size_t j, vector<vector<int>>& cache) {
        if (0 <= cache[i][j]) {
            return cache[i][j];
        }
    ...
    int topdn_dp(const vector<int>& s, const vector<int>& t) {
    ...
    int btmup_dp(const vector<int>& s, const vector<int>& t) {
        size_t m = s.size(), n = t.size();
        if (m == 0 || n == 0) {
            return 0;
        }
