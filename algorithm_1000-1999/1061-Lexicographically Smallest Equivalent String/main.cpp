...
class Solution {
public:
    string smallestEquivalentString_UnionFind(const string& A, const string& B, string S) {
        size_t len = A.size();
        ...
        vector<char> roots(26);
        for (const vector<char>& group : ds.sets()) {
            for (char c : group) {
                roots[c - 'a'] = group.front();
            }
        }
    ...
    string smallestEquivalentString(string A, string B, string S) {
        return smallestEquivalentString_UnionFind(A, B, S);
    }
};
