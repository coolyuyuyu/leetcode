...
class Solution {
public:
    int earliestAcq_UnionFind(vector<vector<int>>& logs, int N) {
        struct Log {
            Log(int time_, unsigned char v1_, unsigned char v2_)
                : time(time_)
                , v1(v1_)
                , v2(v2_) {
            }
        ...
        for (const auto& log : logs) {
            pq.emplace(log[0], log[1], log[2]);
        }

        int time = 0;

        //DisjointSets<unsigned char> ds;
        DisjointSets<unsigned char, SequenceWrapper<vector<unsigned char>>> ds;
        ...
        while (1 < ds.size() && !pq.empty()) {
            ...
            ds.merge(log.v1, log.v2);
        }

        return (ds.size() == 1 ? time : -1);
    }

    int earliestAcq(vector<vector<int>>& logs, int N) {
        return earliestAcq_UnionFind(logs, N);
    }
};
