...
class Solution {
public:
    int maximumMinimumPath_UnionFind(vector<vector<int>>& A) {
        unsigned short rowCnt = A.size();
        unsigned short colCnt = A.empty() ? 0 : A.front().size();
        assert(0 < rowCnt && 0 < colCnt);

        function<int(unsigned short, unsigned short)> getId = [&rowCnt, &colCnt](unsigned short row, unsigned short col) {
            return row * colCnt + col;
        };
        unsigned short srcId = getId(0, 0), dstId = getId(rowCnt - 1, colCnt - 1);
        ...
        struct Node {
            ...
            Node(unsigned short row_, unsigned short col_, int val_)
                : row(row_)
                , col(col_)
                , val(val_) {
            }
        ...
        for (int row = 0; row < rowCnt; ++row) {
            for (int col = 0; col < colCnt; ++col) {
                pq.emplace(row, col, A[row][col]);
            }
        }

        int score = max(A[0][0], A[rowCnt - 1][colCnt - 1]);

        //DisjointSets<unsigned short> ds;
        DisjointSets<unsigned short, SequenceWrapper<vector<unsigned short>>, FindFullCompress<SequenceWrapper<vector<unsigned short>>>> ds;
        while (!ds.connected(srcId, dstId)) {
            Node node = pq.top();
            pq.pop();

            score = node.val;

            unsigned short id1 = getId(node.row, node.col);
            ds.make_set(id1);

            if (0 < node.col) { // lft
                unsigned short id2 = getId(node.row, node.col - 1);
                if (ds.contain(id2)) {
                    ds.merge(id1, id2);
                }
            }
            if (0 < node.row) { // top
                unsigned short id2 = getId(node.row - 1, node.col);
                if (ds.contain(id2)) {
                    ds.merge(id1, id2);
                }
            }
            if ((node.col + 1) < colCnt) { // rht
                unsigned short id2 = getId(node.row, node.col + 1);
                if (ds.contain(id2)) {
                    ds.merge(id1, id2);
                }
            }
            if ((node.row + 1) < rowCnt) { // btm
                unsigned short id2 = getId(node.row + 1, node.col);
                if (ds.contain(id2)) {
                    ds.merge(id1, id2);
                }
    ...
    int maximumMinimumPath(vector<vector<int>>& A) {
        return maximumMinimumPath_UnionFind(A);
    }
};
