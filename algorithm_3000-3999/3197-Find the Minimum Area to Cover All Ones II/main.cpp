    ...
    int minimumSum(vector<vector<int>>& grid) {
        ...
        for (int i = 1; i < m; ++i) {
            for (int j = i + 1; j < m; ++j) {
                ...
                int a2 = minimumArea(grid, i, 0, j - 1, n - 1);
                int a3 = minimumArea(grid, j, 0, m - 1, n - 1);
                ret = std::min(ret, a1 + a2 + a3);
            }
        }
        for (int i = 1; i < m; ++i) {
            for (int j = 1; j < n; ++j) {
        ...
        for (int i = 1; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                int a1 = minimumArea(grid, 0, 0, m - 1, i - 1);
                int a2 = minimumArea(grid, 0, i, m - 1, j - 1);
